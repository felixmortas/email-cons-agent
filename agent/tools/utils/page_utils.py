...
async def look_for_any_captcha(page):
    """
    Deterministic CAPTCHA detection.
    Returns a formal warning if a blocker is detected.
    """
    # List of Common Selectors and Patterns
    captcha_patterns = [
        "iframe[src*='hcaptcha']",
    ...
    for selector in captcha_patterns:
        try:
            # We check whether the element is present and visible
    ...
    if found:
        return (
            ...
            "Utilise l'outil approprié pour interrompre le graph (`stop_execution`)."
        )
    return ""